...
public class Skidmarks : MonoBehaviour {
	...
	private Vector2 vZero = new Vector2(0, 0);
	private Vector2 vX = new Vector2(1, 0);
	private Vector2 vY = new Vector2(0, 1);
	private Vector2 vXY = new Vector2(1, 1);
	// Variables for each mark created. Needed to generate the correct mesh.
	class markSection{
		public Vector3 pos = Vector3.zero;
		public Vector3 normal = Vector3.zero;
		public Vector3 tangent = Vector4.zero;
...
//		int[] triangles = new int[segmentCount * 6];
		...
		for(int i = 0; i < numMarks && i < maxMarks; i++)
			if(skidmarks[i].lastIndex != -1 && skidmarks[i].lastIndex > numMarks - maxMarks)
			{
				...
				uvs[segmentCount * 4 + 0] = vZero;
				uvs[segmentCount * 4 + 1] = vX;
				uvs[segmentCount * 4 + 2] = vY;
				uvs[segmentCount * 4 + 3] = vXY;
